...
class testEnv(gym.Env):
    ...
    def visualize(self):
      ...
      for i in range(self.num_nodes):
          # Determine color based on honeypot position
          color = 'green' if self.honeypot_positions[i] == 1 else 'red'
          
          # Scatter plot for nodes with vulnerabilities
          plt.scatter(i, 1, s=100 * self.vulnerabilities[i], color=color)
          plt.text(i, 1.1, f"{self.vulnerabilities[i]:.2f}", ha='center', fontsize=9)  # Vulnerability text
          
      # Mark attacker position
      plt.scatter(self.attacker_position, 1, color='black', marker='x', s=200, label="Attacker")

      # Plot axis and aesthetics
      plt.plot(range(self.num_nodes), [1] * self.num_nodes, 'k--', linewidth=0.5)
      plt.ylim(0.7, 1.3)  # Adjusted y-limits to accommodate IP addresses
      node_labels = [f"Node {i+1}\n192.168.1.{i+1}" for i in range(self.num_nodes)]
      plt.xticks(range(self.num_nodes),node_labels)  # Node labels
      plt.yticks([])
